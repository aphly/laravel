...
class Snowflake
{
    ...
    public function setStartTimeStamp(int $startTime)
    {
        ...
        if ($missTime < 0) {
            throw new ApiException(['code'=>1,'msg'=>'The start time cannot be greater than the current time']);
        }

        $maxTimeDiff = -1 ^ (-1 << self::MAX_TIMESTAMP_LENGTH);

        if ($missTime > $maxTimeDiff) {
            throw new ApiException(['code'=>1,'msg'=>'The current microtime - starttime is not allowed , You can reset the start time to fix this']);
        }
    ...
    public function getStartTimeStamp()
    {
        ...
        $defaultTime = '2022-01-01 08:00:00';
